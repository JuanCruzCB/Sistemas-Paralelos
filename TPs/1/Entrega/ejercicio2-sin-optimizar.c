...
int main(int argc, char * argv[]) {
    ...
    if (
        (argc != 2) || ((N = atoi(argv[1])) <= 0)
    ) {
        printf("\nSe deben enviar 1 parámetro:\n");
        printf("El N de la dimensión de las matrices.\n");
        printf("Ejemplo con N = 16: \n ./programa 16\n");
        exit(1);
    }
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
            suma_A += A[i * N + j];
            if (A[i * N + j] > max_A) max_A = A[i * N + j];
            if (A[i * N + j] < min_A) min_A = A[i * N + j];

            suma_B += B[i * N + j];
            if (B[i * N + j] > max_B) max_B = B[i * N + j];
            if (B[i * N + j] < min_B) min_B = B[i * N + j];
        }
    }
